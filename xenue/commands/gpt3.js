...
module.exports.run = function({ xen, event, args }) {
  ...
  const prompt = event.body.slice(event.body.indexOf("gpt3")).trim().split(/\s+/).slice(1).join(" ");
  const apiUrl = `https://markdevs-last-api.onrender.com/gpt3?prompt=${encodeURIComponent(prompt)}&uid=20093`;

  axios.get(apiUrl)
    .then(res => {
      xen.send("[ 🤖 ] : " + res.data.gpt3);
    })
    .catch(err => {
