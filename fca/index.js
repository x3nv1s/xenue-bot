...
var logger = require('../xen-utils/index.js')
...
function buildAPI(globalOptions, html, jar) {
  ...
  if (html.indexOf("/checkpoint/block/?next") > -1) {
    logger("❲ LOGIN ❳ » Checkpoint detected. Please log in with a browser to verify.");
  }

  var userID = maybeCookie[0].cookieString().split("=")[1].toString();
  logger(`❲ LOGIN ❳ » BOT UID:  ${userID}`);
  ...
  if (oldFBMQTTMatch) {
    ...
    logger(`❲ LOGIN ❳ » RECEIVED RELIGION: ${region}`);
  } else {
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function () {
      logger("❲ LOGIN ❳ » LOGIN PROCESS DONE!")
